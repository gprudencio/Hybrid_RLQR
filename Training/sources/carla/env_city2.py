...
#        tr = self.low_vel #self.throttle_intensity
#        #st = [self.world.player.get_transform().location.x, self.world.player.get_transform().location.y] #self.steer   
#        st = [self.car_x, self.car_y]
    ...
    def get_done( self ):
        ...
        if (130 < self.car_y < 132 and self.car_x < -5) or \
...
#            cmd = [os.path.join('./sources/carla/', 'CarlaUE4.sh'), '/Game/Carla/Maps/Town01']
            cmd = [os.path.join('./sources/carla/', 'CarlaUE4.sh'), '/Game/Carla/Maps/Town02', \
