...
class source:
    ...
    def __init__( self ):
        ...
        self.avg_rewd = deque()
        self.sum_rewd = 0
        self.sum_long_rewd = 0
        self.sum_lat_rewd = 0
        self.acc_rewd = 0
        self.acc_time = 0

        self.episode_rewards = deque()
        self.episode_long_rewards = deque()
        self.episode_lat_rewards = deque()
    ...
    def verbose( self , episode , rewd , done , avg_length = 10 ):
        ...
        # Transform rewd vector to plot
        long_rewd = rewd[0]
        lat_rewd = np.sum(rewd[1:])
        rewd = np.sum(rewd)

        self.sum_rewd += rewd
        self.sum_long_rewd += long_rewd
        ...
        if done:
            ...
            self.episode_long_rewards.append(self.sum_long_rewd)
            self.episode_lat_rewards.append(self.sum_lat_rewd)
            ...
            plot_lat_long_rewd(self.episode_long_rewards, self.episode_lat_rewards)
