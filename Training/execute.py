...
obsv, n_info  = source.start()
state, info = player.start( source , obsv, n_info )
...
while episode < n_episodes:
    ...
    # Control variables 
    tr, st  = source.ref( )    
    #vel, dx, dy = source.ref( )   # Read reference points to control 
    #vel, dx, dy = 0, 0, 0

    # Run a step and get info
    actn = player.act( state, info, tr, st  )                     # Choose Next Action
    obsv, rewd, env_done, n_info = source.move( actn )   # Run Next Action On Source
    ...
    # Train the algorithm
    state, info = player.learn( state , obsv , actn , rewd , env_done, episode, info, n_info, tr, st  )  # Learn From This Action
    ...
    if env_done:
        obsv, n_info = source.start()
        state, info = player.restart( obsv, n_info )
